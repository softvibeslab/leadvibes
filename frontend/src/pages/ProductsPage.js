...
import {
  ...
  ImagePlus, X, Star, Settings2, ArrowUp, ArrowDown
} from 'lucide-react';
...
export const ProductsPage = () => {
  ...
  const createProductInterest = async () => {
    ...
    try {
      ...
      toast.success('Lead vinculado al producto');
    } catch (error) {
  ...
  const deleteProductInterest = async (interestId) => {
    if (!window.confirm('¿Desvincular este lead del producto?')) return;
    try {
  ...
  const renderEmptyState = () => (
    <Card className="border-dashed border-border/70 bg-card/95">
      <CardContent className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="text-lg font-semibold">No hay productos con esos filtros</h3>
        <p className="mt-2 max-w-md text-sm text-muted-foreground">
          Ajusta tus filtros o crea un producto nuevo para empezar a construir el catálogo.
        </p>
          ...
          {productCustomFields.length > 0 && (
            <div className="space-y-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Aplica datos extra al producto sin tocar el esquema base.</p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
      ...
      <Dialog open={customFieldManagerOpen} onOpenChange={setCustomFieldManagerOpen}>
        <DialogContent className="flex max-h-[90vh] max-w-6xl min-h-0 flex-col overflow-hidden p-0">
          <DialogHeader className="border-b px-6 pb-4 pt-6">
            ...
            <DialogDescription>Administra definiciones para leads y productos desde un solo lugar.</DialogDescription>
          </DialogHeader>
    ...
    <Dialog open={interestDialogOpen} onOpenChange={setInterestDialogOpen}>
      <DialogContent className="flex max-h-[90vh] max-w-5xl min-h-0 flex-col overflow-hidden p-0">
        <DialogHeader className="border-b px-6 pb-4 pt-6">
          ...
          <DialogDescription>
            {selectedProductForInterest ? `Gestiona el interés sobre ${selectedProductForInterest.title}.` : 'Gestiona leads interesados por producto.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-w-0 space-y-4">
          <div className="rounded-xl border border-dashed border-border/70 p-4 space-y-3">
            <p className="text-sm font-medium">Vincular lead a este producto</p>
            ...
            {loadingProductInterests ? (
              ...
              <Card className="border-dashed border-border/70">
                <CardContent className="py-10 text-center text-sm text-muted-foreground">
                  Todavía no hay leads vinculados a este producto.
                </CardContent>
              </Card>
            ) : (
