...
**Telegram/Hermes:** `ROVI_TELEGRAM_BOT_TOKEN`, `ROVI_TELEGRAM_BOT_USERNAME`, `ROVI_TELEGRAM_WEBHOOK_SECRET`, per-role bot usernames (`ROVI_BROKER_TELEGRAM_BOT_USERNAME`, etc.), `TELEGRAM_AGENT_PROFILES` (JSON), `ROVI_HERMES_PROFILES_ROOT`, `ROVI_HERMES_AGENT_BASE_URL`, `ROVI_PUBLIC_API_BASE_URL`. Update ingestion mode: `TELEGRAM_UPDATE_MODE=webhook|polling` (`telegram_polling.py`; polling runs getUpdates loops per bot, no public HTTPS needed — see `docs/TELEGRAM_POLLING.md`).
