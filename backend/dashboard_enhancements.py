...
def _normalized_date_expression(field_name: str) -> dict:
    """
    Normaliza campos de fecha que pueden venir como BSON Date o ISO string.
    """
    ...
    return {
        "$switch": {
            "branches": [
                {
                    "case": {"$eq": [{"$type": field_ref}, "date"]},
                    "then": field_ref
                },
                {
                    "case": {"$eq": [{"$type": field_ref}, "string"]},
                    "then": {
                        "$dateFromString": {
...
def _mixed_date_range_match(field_name: str, start_date: datetime, end_date: datetime | None = None) -> dict:
    """
    Construye un filtro compatible con documentos que guardan fechas mixtas
    como datetime nativo o como string ISO.
    """
    date_range: dict[str, Any] = {"$gte": start_date}
    iso_range: dict[str, Any] = {"$gte": start_date.isoformat()}

    if end_date is not None:
        date_range["$lte"] = end_date
        iso_range["$lte"] = end_date.isoformat()

    return {
        "$or": [
            {field_name: date_range},
            {field_name: iso_range}
        ]
    }
...
async def get_dashboard_trends(
    ...
    ventas_pipeline = [
        {
            "$addFields": {
                "updated_at_normalized": _normalized_date_expression("updated_at")
            }
        },
        {
            "$match": {
                "tenant_id": tenant_id,
                "status": "venta",
                "updated_at_normalized": {"$gte": start_date}
            }
        },
        {
            "$group": {
                "_id": {
                    "year": {"$year": "$updated_at_normalized"},
                    "month": {"$month": "$updated_at_normalized"}
                },
                "ventas": {"$sum": 1},
    ...
    leads_por_fuente_pipeline = [
        {
            "$addFields": {
                "created_at_normalized": _normalized_date_expression("created_at")
            }
        },
        {
            "$match": {
                "tenant_id": tenant_id,
                "created_at_normalized": {"$gte": start_date}
            }
        },
        {
            "$group": {
...
async def get_broker_performance(
    ...
    leads_contactados = await db.leads.count_documents({
        ...
        **_mixed_date_range_match("created_at", current_start)
    })
    ...
    activities_pipeline = [
        ...
        },
        {
            "$match": {
                "tenant_id": tenant_id,
                "broker_id": broker_id,
                "created_at_normalized": {"$gte": current_start}
            }
        },
        {
            "$group": {
    ...
    previous_leads = await db.leads.count_documents({
        "tenant_id": tenant_id,
        "assigned_broker_id": broker_id,
        **_mixed_date_range_match("created_at", previous_start, current_start)
    })
...
async def get_period_metrics(
    ...
    ventas = await db.leads.count_documents({
        "tenant_id": tenant_id,
        "status": "venta",
        **_mixed_date_range_match("updated_at", start_date, end_date)
    })
    ...
    leads_nuevos = await db.leads.count_documents({
        "tenant_id": tenant_id,
        **_mixed_date_range_match("created_at", start_date, end_date)
    })
