...
def create_tasks_router(db: AsyncIOMotorDatabase) -> APIRouter:
    ...
    async def add_comment(task_id: str, payload: TaskCommentCreate, current_user: dict = Depends(get_current_user)):
    ...
    async def seed_demo_tasks(current_user: dict = Depends(get_current_user)):
        tenant_id = current_user["tenant_id"]
